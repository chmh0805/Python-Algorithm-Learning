...
# 검색 알고리즘(Search Algorithm) : 주어진 데이터에서 특정 데이터를 찾는 알고리즘
...
def main():
    ...
    search = 3 # 검색할 데이터
    flag = False # 플래그 변수 : 찾으면 True, 찾지 못하면 False를 저장하는 공간
    index = -1 # 인덱스 변수 : 찾은 위치를 담는 공간

    # [2] Process : 이진 검색(Binary Search) 알고리즘 : 순차검색(Full Scan) / 이진검색(Index Scan)
    low = 0 # min : 낮은 인덱스
    high = N - 1 # max : 높은 인덱스
    while low <= high :
